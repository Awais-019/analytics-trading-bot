<script lang="ts" setup>
  const openNav = ref(false);
  function toggle() {
    openNav.value = !openNav.value;
  }
</script>

<template>
  <header>
    <div
      class="app-container py-7 tablet:grid tablet:grid-flow-col tablet:grid-cols-nav tablet:items-center"
    >
      <div class="mobile:flex inline-block">
        <div>
          <nuxt-link to="/">
            <img src="~/assets/images/logo.svg" alt="Logo" />
          </nuxt-link>
        </div>
        <button
          class="hamburger-menu tablet:hidden"
          :class="{ hidden: openNav }"
          @click="toggle"
        >
          <icons-hamburger />
        </button>
        <button
          class="close tablet:hidden"
          :class="[openNav ? '' : 'hidden']"
          @click="toggle"
        >
          <icons-close />
        </button>
      </div>
      <div
        class="nav"
        :class="[openNav ? 'block' : 'hidden', 'tablet:inline-block']"
      >
        <nav>
          <ul class="nav__list btn-md desktop:text-h4-reg mobile:text-base-reg">
            <li class="nav__item">
              <nuxt-link to="/">Home</nuxt-link>
            </li>
            <li class="nav__item">
              <nuxt-link to="/cryptocurrencies">Cryptocurrucies</nuxt-link>
            </li>
            <li class="nav__item">
              <nuxt-link to="/swap">Swap</nuxt-link>
            </li>
            <li class="nav__item">
              <nuxt-link to="/about">Exchange</nuxt-link>
            </li>
            <ul class="nav__list-sub">
              <li class="nav__item signup btn-sb">
                <nuxt-link to="/signup">Signup</nuxt-link>
              </li>
              <li class="nav__item login btn-sb">
                <nuxt-link to="/signin">Signin</nuxt-link>
              </li>
            </ul>
          </ul>
        </nav>
      </div>
    </div>
  </header>
</template>

<style lang="postcss" scoped>
  header {
    @apply border-[1px] border-primary-white border-opacity-[5%];
  }
  .hamburger-menu,
  .close {
    @apply ml-auto;
  }
  .nav {
    @apply mobile:h-[100dvh] mobile:py-7;
  }
  .nav__list {
    @apply text-neutrals-gray tablet:text-sm tablet:flex tablet:gap-6;
  }
  .nav__item {
    @apply py-3;
  }
  .nav__item a {
    @apply py-2 inline;
  }
  .nav__item a:hover {
    @apply text-neutrals-vista-white;
  }

  .router-link-active {
    @apply text-neutrals-vista-white;
  }

  .nav__list-sub {
    @apply mobile:mt-8 text-center tablet:ml-auto tablet:flex tablet:flex-row-reverse tablet:gap-6;
  }
  .signup {
    @apply bg-primary-tango text-neutrals-vista-white rounded-lg mobile:mb-6 tablet:px-6;
  }
  .signup a:hover {
    @apply border-none;
  }
  .login {
    @apply text-primary-wafer-neu;
  }
  .login a:hover {
    @apply border-none;
  }
</style>
